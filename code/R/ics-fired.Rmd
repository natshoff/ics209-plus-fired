...
ics.conus.grp <- ics.conus %>% 
  filter(STR_DESTROYED_RES_TOTAL > 0) %>%
  group_by(STUSPS) %>%
  summarize(str_des_res = sum(STR_DESTROYED_RES_TOTAL))
# Top 15
top20 <-  ics.conus.grp %>% slice_max(str_des_res, n=10)
# Plot
ggplot(data = ics.conus.grp %>% filter(STUSPS %in% top20$STUSPS), 
       aes(x=reorder(STUSPS, -str_des_res), y=str_des_res)) +
...
```{r}
states.sub <- c("AZ", "CO", "NV", "WY", "CA", "ID", "WA", "OR", "NM", "MT", "UT", "TX", "OK")
ics.west.plus <- ics.conus %>% filter(STUSPS %in% states.sub)
## Subset FIRED as well
fired.west.plus <- fired.conus %>% filter(STUSPS %in% states.sub)
## Homes destroyed
