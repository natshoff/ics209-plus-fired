...
tt.fired.df <- tt %>% dplyr::select(id, INCIDENT_ID) %>% as_tibble() %>%
  dplyr::select(-geom) %>%
  inner_join(fired.west.plus%>%as_tibble(), ., by="id", left=F) %>%
  inner_join(., ics.west.plus%>%as_tibble()%>%dplyr::select(-STUSPS, -geom), by="INCIDENT_ID") %>%
  dplyr::select(-X.1)

## Join back to df
dim(min.filter_%>%filter(INCIDENT_ID %in% tt.fired.df$INCIDENT_ID))
## Join for the final df
final.join <- bind_rows(min.filter_, tt.fired.df) %>%
  select(-c(doy_diff, ar_abs_dif, ar_chg_dif, STUSPS.y))
...
Re-attach state boundaries.

```{r}
## Re-attach state boundaries
ics.fired <- 
  st_join(ics.fired%>%dplyr::select(-STUSPS), 
          states%>%dplyr::select(STUSPS, geom), join=st_intersects, largest=T)
```
