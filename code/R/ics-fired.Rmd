...
# write this file out
st_write(mtbs.inci, "../../data/spatial/mod/ICS_PLUS_FIRED/mtbs_perims_w_incidents.gpkg", delete_dsn=T)
...
# Write to file
st_write(events.ics,"../../data/spatial/mod/ICS_PLUS_FIRED/ics209plus_fired_by_mtbs.gpkg",delete_dsn=TRUE)
...
# Write to file
st_write(events.ics2,"../../data/spatial/mod/ICS_PLUS_FIRED/ics209plus_fired_by_buffer_cmplx.gpkg",delete_dsn=TRUE)
...
# Write to file
st_write(events.ics3,"../../data/spatial/mod/ICS_PLUS_FIRED/ics209plus_fired_by_buffer_no_cmplx.gpkg",delete_dsn=TRUE)
...
for (y in 1:length(years)){
 ...
 st_agr(b) = "constant"
 join[[y]] <- st_join(b,a,st_nn,k=50,left=TRUE)
}
...
# Write to file
st_write(events.ics4,"../../data/spatial/mod/ICS_PLUS_FIRED/ics209plus_fired_by_buffer_remaining.gpkg",delete_dsn=TRUE)

# Tidy
rm(buffer,join,df,a,b)
```
...
events.ics.master <- bind_rows(events.ics.master,events.ics4)

rm(events.ics4)

incident.pts_ <- incident.pts_ %>%
 filter(!INCIDENT_ID %in% events.ics.master$INCIDENT_ID)

# Check for duplicate records ...
dim(events.ics.master %>% group_by(INCIDENT_ID) %>% filter(n()>1))[1]
dim(events.ics.master %>% group_by(FIRED_ID) %>% filter(n()>1))[1]

# Handle duplicate records
events.ics.master <- events.ics.master %>% distinct(., INCIDENT_ID, .keep_all=TRUE)
...
st_write(events.ics.master, "../../data/spatial/mod/ICS_PLUS_FIRED/ics209plus_fired_combined.gpkg",
         delete_dsn=TRUE)
